...
public interface RecordMapper {
  ...
  @Select(
      "<script><choose><when test='mask!=null'><if test='mask.reid or mask.reimid or mask.reemid or mask.reqty'><trim prefix='SELECT' suffixOverrides=','><if test='mask.reid'>REID, </if><if test='mask.reimid'>REIMID, </if><if test='mask.reemid'>REEMID, </if><if test='mask.reqty'>REQTY, </if></trim>FROM F4211View WHERE REID=#{reid}</if></when><otherwise>SELECTREID,REIMID,REEMID,REQTY FROM F4211View WHERE REID=#{reid}</otherwise></choose></script>")
  ...
  @Insert(
      "INSERT INTO F4211 (REID,REIMID,REEMID,REQTY) VALUES (#{bean.reid,jdbcType=INTEGER},#{bean.reimid,jdbcType=INTEGER},#{bean.reemid,jdbcType=SMALLINT},#{bean.reqty,jdbcType=SMALLINT})")
  void add(@Param("bean") Record bean);

  @Update(
      "<script><choose><when test='mask!=null'><if test='mask.reimid or mask.reemid or mask.reqty'>UPDATE F4211 <set><if test='mask.reimid'>REIMID=#{bean.reimid,jdbcType=INTEGER}, </if><if test='mask.reemid'>REEMID=#{bean.reemid,jdbcType=SMALLINT}, </if><if test='mask.reqty'>REQTY=#{bean.reqty,jdbcType=SMALLINT}, </if></set>WHERE REID=#{bean.reid}</if></when><otherwise>UPDATE F4211 SET REIMID=#{bean.reimid,jdbcType=INTEGER}, REEMID=#{bean.reemid,jdbcType=SMALLINT}, REQTY=#{bean.reqty,jdbcType=SMALLINT} WHERE REID=#{bean.reid}</otherwise></choose></script>")
