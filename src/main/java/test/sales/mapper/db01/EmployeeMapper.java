...
public interface EmployeeMapper {
  ...
  @Select(
      "<script><choose><when test='mask!=null'><if test='mask.emid or mask.emstatus or mask.emgender or mask.emname or mask.emamount or mask.embirthday'><trim prefix='SELECT' suffixOverrides=','><if test='mask.emid'>EMID, </if><if test='mask.emstatus'>EMSTATUS, </if><if test='mask.emgender'>EMGENDER, </if><if test='mask.emname'>EMNAME, </if><if test='mask.emamount'>EMAMOUNT, </if><if test='mask.embirthday'>EMBIRTHDAY, </if></trim>FROM F0101VIEW WHERE EMID=#{emid}</if></when><otherwise>SELECTEMID,EMSTATUS,EMGENDER,EMNAME,EMAMOUNT,EMBIRTHDAY FROM F0101VIEW WHERE EMID=#{emid}</otherwise></choose></script>")
  Employee get(@Param("emid") Integer emid, @Param("mask") EmployeeMask mask);

  @Insert(
      "INSERT INTO F0101 (EMID,EMSTATUS,EMGENDER,EMNAME,EMBIRTHDAY) VALUES (#{bean.emid,jdbcType=SMALLINT},#{bean.emstatus,jdbcType=SMALLINT},#{bean.emgender,jdbcType=VARCHAR},#{bean.emname,jdbcType=VARCHAR},#{bean.embirthday,jdbcType=TIMESTAMP})")
  void add(@Param("bean") Employee bean);

  @Update(
      "<script><choose><when test='mask!=null'><if test='mask.emstatus or mask.emgender or mask.emname or mask.embirthday'>UPDATE F0101 <set><if test='mask.emstatus'>EMSTATUS=#{bean.emstatus,jdbcType=SMALLINT}, </if><if test='mask.emgender'>EMGENDER=#{bean.emgender,jdbcType=VARCHAR}, </if><if test='mask.emname'>EMNAME=#{bean.emname,jdbcType=VARCHAR}, </if><if test='mask.embirthday'>EMBIRTHDAY=#{bean.embirthday,jdbcType=TIMESTAMP}, </if></set>WHERE EMID=#{bean.emid}</if></when><otherwise>UPDATE F0101 SET EMSTATUS=#{bean.emstatus,jdbcType=SMALLINT}, EMGENDER=#{bean.emgender,jdbcType=VARCHAR}, EMNAME=#{bean.emname,jdbcType=VARCHAR}, EMBIRTHDAY=#{bean.embirthday,jdbcType=TIMESTAMP} WHERE EMID=#{bean.emid}</otherwise></choose></script>")
  ...
  @Select(
      "<script>SELECT DECODE(MIN(EMBIRTHDAY),NULL,${defaultValue}) FROM F0101VIEW<if test='filter!=null'> WHERE ${filter.toStringOracle()}</if></script>")
  java.util.Date minEmbirthday(
      @Param("filter") FilterExpr filter, @Param("defaultValue") java.util.Date defaultValue);

  @Select(
      "<script>SELECT DECODE(MAX(EMBIRTHDAY),NULL,${defaultValue}) FROM F0101VIEW<if test='filter!=null'> WHERE ${filter.toStringOracle()}</if></script>")
  java.util.Date maxEmbirthday(
      @Param("filter") FilterExpr filter, @Param("defaultValue") java.util.Date defaultValue);
}
